...
export const avatarSourceSizeError = (byteLength: number): string | null => byteLength > AVATAR_SOURCE_MAX_BYTES ? uiCopy("Choose an image smaller than 25 MB.") : null;
...
export function buildImageContextMenuTemplate(contents: { copyImageAt(x: number, y: number): void; undo(): void; redo(): void; cut(): void; copy(): void; paste(): void; selectAll(): void }, params: ImageContextMenuParams, deps: { openExternalUrl(url: string): Promise<unknown>; copyText(text: string): void; imageActions: ImageContextActions }): ImageContextMenuItem[] {
  ...
  if (params.linkURL.length > 0) sections.push([{ label: uiCopy("Open link"), click: () => { void deps.openExternalUrl(params.linkURL); } }, { label: uiCopy("Copy link address"), click: () => deps.copyText(params.linkURL) }]);
  if (params.mediaType === "image") { const items: ImageContextMenuItem[] = [{ label: uiCopy("Copy image"), click: deps.imageActions.copyImage }, { label: uiCopy("Save image…"), click: () => { void deps.imageActions.saveImage(); } }]; if (params.srcURL.length > 0 && deps.imageActions.copyImageAddress != null) items.push({ label: uiCopy("Copy image address"), click: deps.imageActions.copyImageAddress }); sections.push(items); }
  if (params.isEditable) sections.push([{ label: uiCopy("Undo"), accelerator: "CmdOrCtrl+Z", enabled: params.editFlags.canUndo === true, click: () => contents.undo() }, { label: uiCopy("Redo"), accelerator: "Shift+CmdOrCtrl+Z", enabled: params.editFlags.canRedo === true, click: () => contents.redo() }, { label: uiCopy("Cut"), accelerator: "CmdOrCtrl+X", enabled: params.editFlags.canCut === true, click: () => contents.cut() }, { label: uiCopy("Copy"), accelerator: "CmdOrCtrl+C", enabled: params.editFlags.canCopy === true, click: () => contents.copy() }, { label: uiCopy("Paste"), accelerator: "CmdOrCtrl+V", enabled: params.editFlags.canPaste === true, click: () => contents.paste() }, { label: uiCopy("Select All"), accelerator: "CmdOrCtrl+A", enabled: params.editFlags.canSelectAll === true, click: () => contents.selectAll() }]); else if (params.selectionText.length > 0) sections.push([{ label: uiCopy("Copy"), accelerator: "CmdOrCtrl+C", click: () => contents.copy() }]);
...
export function registerImageContextMenu(deps: ImageContextMenuElectronPorts): void {
  ...
  const saveImageFromContextMenu = async (contents: ImageContextMenuContents, params: ImageContextMenuParams): Promise<void> => {
    try {
      ...
      const result = await deps.dialog.showSaveDialog(owner, {
        ...
        filters: [{ name: uiCopy("Images"), extensions: ["png", "jpg", "jpeg", "gif", "webp", "svg", "bmp", "avif"] }],
      });
  ...
  const buildImageContextMenu = (contents: ImageContextMenuContents, params: ImageContextMenuParams): { readonly items: readonly unknown[]; append(item: unknown): void; popup(options: { readonly window?: unknown }): void } => {
    ...
    if (hasLink) {
      sections.push([
        new deps.MenuItem({ label: uiCopy("Open link"), click: () => void deps.openExternalUrl(linkUrl) }),
        new deps.MenuItem({ label: uiCopy("Copy link address"), click: () => deps.clipboard.writeText(linkUrl) }),
      ]);
    }
    if (hasImage) {
      const imageItems: unknown[] = [
        new deps.MenuItem({ label: uiCopy("Copy image"), click: () => copyImageFromContextMenu(contents, params) }),
        new deps.MenuItem({ label: uiCopy("Save image…"), click: () => void saveImageFromContextMenu(contents, params) }),
      ];
      if (mediaUrl.length > 0) imageItems.push(new deps.MenuItem({ label: uiCopy("Copy image address"), click: () => deps.clipboard.writeText(mediaUrl) }));
      sections.push(imageItems);
    }
    if (params.isEditable) {
      sections.push([
        new deps.MenuItem({ label: uiCopy("Undo"), accelerator: "CmdOrCtrl+Z", enabled: params.editFlags.canUndo === true, click: () => contents.undo() }),
        new deps.MenuItem({ label: uiCopy("Redo"), accelerator: "Shift+CmdOrCtrl+Z", enabled: params.editFlags.canRedo === true, click: () => contents.redo() }),
        new deps.MenuItem({ label: uiCopy("Cut"), accelerator: "CmdOrCtrl+X", enabled: params.editFlags.canCut === true, click: () => contents.cut() }),
        new deps.MenuItem({ label: uiCopy("Copy"), accelerator: "CmdOrCtrl+C", enabled: params.editFlags.canCopy === true, click: () => contents.copy() }),
        new deps.MenuItem({ label: uiCopy("Paste"), accelerator: "CmdOrCtrl+V", enabled: params.editFlags.canPaste === true, click: () => contents.paste() }),
        new deps.MenuItem({ label: uiCopy("Select All"), accelerator: "CmdOrCtrl+A", enabled: params.editFlags.canSelectAll === true, click: () => contents.selectAll() }),
      ]);
    } else if (params.selectionText.length > 0) {
      sections.push([new deps.MenuItem({ label: uiCopy("Copy"), accelerator: "CmdOrCtrl+C", click: () => contents.copy() })]);
    }
