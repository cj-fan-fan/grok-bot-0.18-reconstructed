  ...
  const moveDisposition = await moveToApplicationsFolderIfNeeded({
    ...
    confirmMove: async () => {
      const result = await deps.dialog.showMessageBox({
        type: "question",
        title: uiCopy("Move Grok Bot to Applications"),
        message: uiCopy("Move Grok Bot to the Applications folder?"),
        detail: uiCopy("Grok Bot cannot install updates from its current location. It will reopen after moving."),
        buttons: [uiCopy("Move to Applications"), uiCopy("Not Now")],
        defaultId: 0,
        cancelId: 1,
      });
    ...
    reportFailure: async (error) => {
      ...
      await deps.dialog.showMessageBox({
        type: "error",
        title: uiCopy("Couldn't Move Grok Bot"),
        message: uiCopy("Grok Bot couldn't move to Applications"),
        detail: uiCopy("Move Grok Bot to the Applications folder manually, then reopen Grok Bot"),
        buttons: [uiCopy("OK")],
        defaultId: 0,
      });
    },
